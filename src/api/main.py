...
from schema import Author, AuthorBase, AuthorCreate, AuthorUpdate, BookBase, Book, BookCreate, BookUpdate, UserBase, Token
...
async def authors(
    ...
    return paginate(db.session, select(AuthorModel).filter(AuthorModel.name.ilike(f'%{search}%')).order_by(asc(AuthorModel.id)))
...
async def update_author(author_id, author: AuthorUpdate):
...
async def create_book(book: BookCreate):
    new_book = BookModel(name=book.name, number_pages=book.number_pages, author_id=book.author_id)
...
async def update_book(book_id, book: BookUpdate):
